...
autism_screening%>% group_by(autism) %>% summarise(c = n())
```

## b) 
Does there seem to a relation between gender and the prevalence of autism? Investigate this hypothesis with a suitable graphical display of data and a numerical summary. 

**Your answer:**

```{r}
ggplot(autism_screening) + 
  geom_bar(aes(x= gender, fill = autism))


autism_screening %>% group_by(gender,autism)  %>% summarise(c = n())
```
...
```{r}
fit1 <- lm(age ~ autism, data = autism_screening)
summary(fit1)

ggplot(data = autism_screening) + 
  geom_boxplot(mapping = aes(x = autism, y = age))
```
...
```{r}
autism_screening %>% group_by(contry_of_res) %>% summarise(n()) 
```
...
```{r}
autism_screening  %>% mutate(aut1 =  ifelse(autism == "yes", 1, 0)) %>% group_by(ethnicity) %>%
   summarise(mean(aut1))
  
```

## g)
Columns 1-10 represent scores on a questionnaire used by health professionals to screen for autism. Does the total score `total` represent a useful information for predicting the presence of autism? What plots are useful to get a tentative answer? What formal statistical tools can you use to answer this question?  
How do you asses the significance of your findings? 

**Your answer:**
```{r}
fit2 <- lm(result ~ autism, data = autism_screening)
summary(fit2)

ggplot(autism_screening)+
  geom_boxplot(mapping = aes(x = autism, y = result))
```
...
**Your answer**:
If we sell more `FreshNoodles` than `FreshCutPasta` then the response variable is equal to 1 by its definition. We are asked to compute the probability, according to the model, of the response being equal to 1 when `flavour` is equal to either `wheat` or `whole wheat`. Those probabilities are equal to the logistic function for each category of `flavour`
...
It is more likely to sell more `FreshNoodles` than `FreshCutPasta`, i.e. $Y=1$, when the ingredient is just wheat.
...
$$
  where $\beta_j$ is the parameter associated to covariate $X_j$ and $\lambda$ is the penalty tuning parameter in the Lasso regression. Since `r round(abs(out$e1)+abs(out$e2),3)` is greater than `r out$pen` we have that the provided value of the penalty parameter does not allow to get those estimates.
...
It is a good pratice to standardize the explanatory variables prior to estimate the models as to ensure that the penalty is sensible (the actual value of coefficient estimates depend on the predictor's scale). In this case the variability of the two predictors do not differ too much, so the variables could be used as they are. That being said, it is always good pratice to standardize the predictors (as it can be time consuming to assess whether or not predictors have to be standardized when the number of predictors is large).
...
```{r echo=TRUE}
library(glmnet)
#just to check the distribution of the response
table(leukemia$Y)
#roughly 1/3 of the tissue sample contain cancer cells


leukemia$Y <- as.factor(leukemia$Y)
#create model matrix (covariates) and the response
which(names(leukemia)=="Y")
#the response is in the first column
##we scale the predictors
X <- scale(as.matrix(leukemia[,-1]))
y <- leukemia$Y

#create a test set to evaluate the performance of the classifier later on
ids <- sample(1:nrow(leukemia), round(nrow(leukemia)*0.8), replace=FALSE)
...
#check the distribution of the response in the training set and test set
table(y.train)
table(y.test)


#go with the Lasso as to perform variable selection as we have a large number of variables
#run cross validation to select the best model
fitlassocv <- cv.glmnet(X.train, y.train, alpha = 1, family="binomial")
#choose the best model according to the one standard deviation rule. Do not display the coefficients: too large output
my.coefs <- coef(fitlassocv, s = "lambda.1se")
```
...
**Your answer**:
```{r echo=TRUE}
preds <- predict(fitlassocv, X.test, s = "lambda.1se", type="class")
#create the confusion matrix
(tab <- table(preds, y.test))

library(pROC)
preds.resp <- predict(fitlassocv, X.test, s = "lambda.1se", type="response")
mul_roc <- roc(y.test ~ preds.resp,plot=TRUE,print.auc=TRUE)
```

By looking at the confusion matrix the predictions are in fact oracle predictions. The overall performance of the classifier is excellent as the ROC curve shows that for any given threshold set to classify the observations (AUC=1).
