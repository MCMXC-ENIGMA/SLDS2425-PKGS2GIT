inst.libs <- function(pkgs=NULL){
	 if(is.null(pkgs)){
	 	pkgs <- c("tidyverse", "ISLR", "FNN", "glmnet", "kernlab", "pROC", "openintro", "BHH2", "rmarkdown")
	 }else{
	 	pkgs <- c(pkgs, c("tidyverse", "ISLR", "FNN", "glmnet", "kernlab", "pROC", "openintro", "BHH2", "rmarkdown"))
	 }
...
my.fun <- function(matricola){
  ...
  beta <- runif(1, 20, 50)
  x <- rbinom(n, 1,0.5)
  pasta <- ifelse(x==0, "Fresh Noodles","Fresh Cut Pasta")
  MEANS <- sample(c(1e1,3e1))
  x <- ifelse(x==0, MEANS[1], MEANS[2])
  pasta <- as.factor(pasta)
  SD <- runif(1, 5e1, 8e1)
  sales <- rnorm(n, mean=beta*x, sd=SD)
  sales <- ifelse(sales<0,0, sales)
  fit <- lm(sales~pasta)
  list(model=fit, covariate=pasta, residuals=resid(fit))
}


vltr.fun <- function(matricola){
  out <- my.fun(matricola)
  summ <- summary(out$model)
  MY.print.summary.lm(summ)
  list(pasta=out$covariate, residuals=out$residuals)
}
...
function (x, digits = max(3L, getOption("digits") - 3L), symbolic.cor = x$symbolic.cor,
          signif.stars = getOption("show.signif.stars"), concise = FALSE, ...)
    {
        ...
        if (length(x$aliased) == 0L) {
            ...
            MYprintCoefmat(coefs, digits = digits, signif.stars = signif.stars, signif.legend = (!concise),
